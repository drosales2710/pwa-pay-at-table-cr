...
} from "../../utils/restaurantConfig"
import type { GuardianMode } from "../../types"

function Section({ title, titleEn, children }: { title: string; titleEn: string; children: ReactNode }) {
  return (
    ...
    <div style={{ marginBottom: "16px" }}>
      ...
      <div style={{ display: "flex", gap: "8px" }}>
        <input
          ...
          style={{ flex: 1, padding: "9px 12px", border: "1px solid #E2E8F0", borderRadius: "8px", fontSize: "0.85rem", color: "#0F172A", outline: "none", boxSizing: "border-box", fontFamily: secret ? "monospace" : "inherit" }}
        />
        {secret && (
...
export default function SystemSettings() {
  const { restaurant, saveRestaurantSettings } = useGuest()
  const [saved, setSaved] = useState(false)
  const [name, setName] = useState(restaurant.name)
  const [tagline, setTagline] = useState(restaurant.tagline)
  const [rate, setRate] = useState(String(restaurant.exchangeRate))
  const [taxEnabled, setTaxEnabled] = useState(restaurant.taxConfig.enabled)
  const [guardianOn, setGuardianOn] = useState(restaurant.guardianConfig.mode !== "off")
  const [guardianMode, setGuardianMode] = useState<GuardianMode>(
    restaurant.guardianConfig.mode === "off" ? "smart" : restaurant.guardianConfig.mode
  )
  const [brand, setBrand] = useState(restaurant.brand?.primaryColor ?? "#F97316")
  const [sinpePhone, setSinpePhone] = useState(restaurant.sinpe.phone)
  const [sinpeName, setSinpeName] = useState(restaurant.sinpe.merchantName)
  const [contact, setContact] = useState<RestaurantContact>(getRestaurantContact(restaurant.id))
  const [features, setFeatures] = useState<RestaurantFeatureFlags>(getRestaurantFeatures(restaurant.id))
  const [gateway] = useState({ merchantId: "BAC-CR-882341", terminalId: "TERM-0042", apiKey: "sk_live_bac_xxxxxxxxxxxxxxxxxx" })
  ...
  const handleSave = () => {
    saveRestaurantSettings({
      name,
      tagline,
      exchangeRate: Number(rate) || restaurant.exchangeRate,
      taxConfig: { ...restaurant.taxConfig, enabled: taxEnabled },
      guardianConfig: {
        ...restaurant.guardianConfig,
        mode: guardianOn ? guardianMode : "off",
      },
      brand: { primaryColor: brand },
      sinpe: { phone: sinpePhone, merchantName: sinpeName },
      contact,
      features,
    })
    setSaved(true)
    ...
    <div>
      ...
      <Section title="Restaurante" titleEn="Restaurant Info">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
          <Field label="Nombre" labelEn="Name" value={name} onChange={setName} />
          <Field label="Teléfono" labelEn="Phone" value={contact.phone} onChange={(v) => setContact((c) => ({ ...c, phone: v }))} />
          <Field label="Eslogan" labelEn="Tagline" value={tagline} onChange={setTagline} />
          <Field label="Correo electrónico" labelEn="Email" value={contact.email} type="email" onChange={(v) => setContact((c) => ({ ...c, email: v }))} />
        </div>
        <Field label="Dirección" labelEn="Address" value={contact.address} onChange={(v) => setContact((c) => ({ ...c, address: v }))} />
        <Field label="Color de marca" labelEn="Brand color" value={brand} onChange={setBrand} />
      </Section>

      <Section title="Impuestos y Guardian" titleEn="Tax & Guardian">
        <Toggle label="Calcular IVA + servicio (Ley 1802)" labelEn="Calculate IVA + service tax" checked={taxEnabled} onChange={setTaxEnabled} />
        <Toggle label="Guardian Mode (aprobación de pedidos)" labelEn="Order approval queue before KDS" checked={guardianOn} onChange={setGuardianOn} />
        {guardianOn && (
          <div style={{ marginTop: "12px" }}>
            <label style={{ display: "block", fontSize: "0.75rem", fontWeight: 700, color: "#64748B", marginBottom: "5px" }}>Modo / Mode</label>
            <select
              value={guardianMode}
              onChange={(e) => setGuardianMode(e.target.value as GuardianMode)}
      ...
      <Section title="SINPE Móvil" titleEn="SINPE">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
          <Field label="Teléfono comercio" labelEn="Merchant phone" value={sinpePhone} onChange={setSinpePhone} />
          <Field label="Nombre comercio" labelEn="Merchant name" value={sinpeName} onChange={setSinpeName} />
        </div>
      </Section>

      <Section title="Gateway BAC Credomatic" titleEn="Payment Gateway">
        <p style={{ fontSize: "0.78rem", color: "#92400E", background: "#FFF7ED", border: "1px solid #FED7AA", borderRadius: "8px", padding: "10px 14px", marginBottom: "14px" }}>
          Placeholder — no se envía a BAC. La conexión real queda para una iteración posterior. / Not sent to BAC. Live gateway comes later.
        </p>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
          <Field label="Merchant ID" labelEn="Merchant ID" value={gateway.merchantId} onChange={() => undefined} />
          <Field label="Terminal ID" labelEn="Terminal ID" value={gateway.terminalId} onChange={() => undefined} />
        </div>
        <Field label="API Key (producción)" labelEn="Production API Key" value={gateway.apiKey} onChange={() => undefined} secret />
      </Section>
      ...
      <Section title="Facturación Electrónica (DGT)" titleEn="Electronic Invoicing — Hacienda">
        <p style={{ fontSize: "0.78rem", color: "#64748B", marginBottom: "10px" }}>
          Placeholder — Hacienda XML no se emite aún. / Placeholder — Hacienda XML is not issued yet.
        </p>
      ...
      <Section title="Tipo de cambio" titleEn="Exchange Rate">
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <button type="button" disabled style={{ display: "flex", alignItems: "center", gap: "6px", padding: "9px 14px", borderRadius: "8px", border: "1px solid #E2E8F0", background: "#F8FAFC", fontSize: "0.8rem", fontWeight: 600, color: "#94A3B8", marginTop: "16px" }}>
            <RefreshCw size={13} /> BCCR — próximamente / coming soon
          </button>
        </div>
      </Section>
      ...
      <Section title="Funcionalidades" titleEn="Feature Flags">
        ...
        <p style={{ fontSize: "0.7rem", color: "#94A3B8", marginTop: "8px" }}>
          Solo “Doble moneda” y Guardian/impuestos cambian el guest UI ahora. / Only dual currency, tax, and Guardian affect the guest UI now.
        </p>
      </Section>
    </div>
  )
